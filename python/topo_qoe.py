...
class QoETopo(Topo):
    def build(self):
        ...
        s2 = self.addSwitch('s2')   # Path A core switch 1
        s5 = self.addSwitch('s5')   # Path A core switch 2 (extra hop)
        s3 = self.addSwitch('s3')   # Path B core switch
        ...
        # --- Path A: s1 → s2 → s5 → s4 (3 hops, more latency) ---
        self.addLink(s1, s2, bw=10)
        self.addLink(s2, s5, bw=10)
        self.addLink(s5, s4, bw=10)

        # --- Path B: s1 → s3 → s4 (2 hops, less latency) ---
